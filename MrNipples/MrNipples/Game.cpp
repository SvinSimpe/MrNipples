...
HRESULT Game::InitializeBasicShaders()
{
	...
	if ( SUCCEEDED( hr = CompileShader( "BasicShader.hlsl", "VS", "vs_5_0", nullptr, &vs ) ) )
	{
	...
	if( SUCCEEDED( hr = CompileShader( "BasicShader.hlsl", "PS", "ps_5_0", nullptr, &tps ) ) )
	{
...
HRESULT Game::Initialize( ID3D11Device* device, ID3D11DeviceContext* deviceContext )
{
	...
	// Just for testing!
	GeometryBox *testBox = new GeometryBox( XMFLOAT3( 0.0f, 0.0f, 0.0f ), 5.0f, 5.0f, 5.0f, 0.25f, 0.8f, 0.5f );
